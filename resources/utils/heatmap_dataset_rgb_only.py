...
from resources.utils.depth_preprocessing import resize_array_depth
...
class DSL:
    ...
    def get_dataset_info(self):
        ...
        for i, batch in tqdm.tqdm(enumerate(data_batch)):
            ...
            if person_ not in self.dataset[mapped_class]:
                self.dataset[mapped_class][person_] = {'rgb':[],'depth':[]}

            for data_point in os.listdir(os.path.join(batch_path, 'heatmap_raw')):
                avi_file = os.path.join(batch_path,'heatmap_raw', data_point)
                depth_file = os.path.join(batch_path,'depth',avi_file)
                
                if  os.path.exists(avi_file) and os.path.exists(depth_file) :
                    self.dataset[mapped_class][person_]['rgb'].append(avi_file)
                    self.dataset[mapped_class][person_]['depth'].append(depth_file)
    ...
    def divide_filter(self,filter_list,ratios:list,randomize=True):
        ...
        #shuffle the dataset
        zipped = list(zip(filter_list['rgb'],filter_list['depth'],filter_list['class']))
        if randomize:
            random.seed(self.random_seed)
            random.shuffle(zipped)
        filtered_list = {'rgb':[],'depth':[],'class':[],'len':0}
        result_list = []
    ...
    def filter(self,classes:list=None,persons:list=None,randomize=True):
        ...
        if randomize:
            random.seed(self.random_seed)
            zipped = list(zip(filtered_list['rgb'],filtered_list['depth'],filtered_list['class']))
            random.shuffle(zipped)
            filtered_list['rgb'],filtered_list['depth'],filtered_list['class'] = zip(*zipped)
    ...
    def get_persons(self):
        return self.persons
    def get_generator(self,filtered_list=None,mode = None,randomize=True,spatial_augument = None,fintuning = None,in_channles = None, **kwargs):
